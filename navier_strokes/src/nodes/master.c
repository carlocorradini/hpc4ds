...
static void send_simulation(int worker, const com_message_t *const message, const ns_simulation_t *const simulation) {
    char *simulation_string = NULL;
    MPI_Datatype message_type;

    simulation_string = ns_stringify_simulation(simulation);
    if (simulation_string == NULL) {
        log_error("Unable to stringify simulation %ld", message->simulation_id);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    com_message_MPI_datatype(&message_type);

    log_info("Sending simulation metadata %ld to worker node %d", message->simulation_id, worker);
    MPI_Send(message, 1, message_type, worker, 0, MPI_COMM_WORLD);
    log_info("Simulation metadata %ld sent", message->simulation_id);

    log_info("Sending simulation %ld to worker node %d", message->simulation_id, worker);
    MPI_Send(simulation_string, (int) strlen(simulation_string) + 1, MPI_CHAR, worker, 0, MPI_COMM_WORLD);
    log_info("Simulation %ld sent", message->simulation_id);
